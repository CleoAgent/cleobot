---
summary: "OpenProse: .prose workflows, slash commands, and state in CleoBot"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In CleoBot it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
Dev/local checkout: `cleobot plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses CleoBot tools under the hood.
...
## CleoBot runtime mapping

OpenProse programs map to CleoBot primitives:

| OpenProse concept         | CleoBot tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use CleoBot tool allowlists and approval gates to control side effects.
