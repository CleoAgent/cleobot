...
CleoBot selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models CleoBot can use (plus aliases).
...
`cleobot models` (no subcommand) is a shortcut for `models status`.
...
`cleobot models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.cleobot/agents/<agentId>/models.json`). This file
