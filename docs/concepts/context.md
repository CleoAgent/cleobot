...
read_when:
  - You want to understand what “context” means in CleoBot
...
“Context” is **everything CleoBot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (CleoBot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How CleoBot builds the system prompt

The system prompt is **CleoBot-owned** and rebuilt each run. It includes:
...
By default, CleoBot injects a fixed set of workspace files (if present):
