...
- `/usage cost` in chats: local cost summary aggregated from CleoBot session logs.
- CLI: `cleobot status --usage` prints a full per-provider breakdown.
- CLI: `cleobot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
