...
Deploy CleoBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
repository configures everything needed to run CleoBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CLEOBOT_STATE_DIR
        value: /data/.openclaw
      - key: CLEOBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: CLEOBOT_GATEWAY_TOKEN
...
If you use the original CleoBot repository, Render will not auto-deploy your CleoBot. To update it, run a manual Blueprint sync from the dashboard.
...
For CleoBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any CleoBot host.
