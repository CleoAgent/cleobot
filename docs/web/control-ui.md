...
If the page fails to load, start the Gateway first: `cleobot gateway`.
...
you revoke it with `cleobot devices revoke --device <id> --role <role>`. See
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. CleoBot
...
CleoBot **blocks** Control UI connections without device identity.
...
```bash
CLEOBOT_CONTROL_UI_BASE_PATH=/openclaw/ pnpm ui:build
```
