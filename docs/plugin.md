---
summary: "CleoBot plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends CleoBot with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core CleoBot yet (or you want to keep optional features out of your main
install).
...
CleoBot plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
- `~/.cleobot/extensions/*.ts`
- `~/.cleobot/extensions/*/index.ts`

4. Bundled extensions (shipped with CleoBot, **disabled by default**)

- `<openclaw>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `cleobot plugins enable <id>`. Installed plugins are enabled by default,
...
CleoBot can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.cleobot/mpm/plugins.json`
- `~/.cleobot/mpm/catalog.json`
- `~/.cleobot/plugins/catalog.json`

Or point `CLEOBOT_PLUGIN_CATALOG_PATHS` (or `CLEOBOT_MPM_CATALOG_PATHS`) at
...
If a plugin exports `id`, CleoBot uses it but warns when it doesn’t match the
configured id.
...
CleoBot augments `uiHints` at runtime based on discovered plugins:
...
openclaw plugins install <path>                 # copy a local file/dir into ~/.cleobot/extensions/<id>
...
Plugins may also register their own top‑level commands (example: `cleobot voicecall`).
...
- Plugin-managed hooks show up in `cleobot hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `cleobot hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside CleoBot (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `cleobot models auth login --provider <id> [--method <id>]`
...
- `config`: The current CleoBot config
...
- `cleobot plugins install <npm-spec>` uses `npm pack`, extracts into `~/.cleobot/extensions/<id>/`, and enables it in config.
...
- CLI: `cleobot voicecall start|status`
- Tool: `voice_call`
