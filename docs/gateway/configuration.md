...
CleoBot reads an optional **JSON5** config from `~/.openclaw/openclaw.json` (comments + trailing commas allowed).

If the file is missing, CleoBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.openclaw/workspace`). You usually only need a config to:
...
CleoBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `cleobot doctor`, `cleobot logs`, `cleobot health`, `cleobot status`, `cleobot service`, `cleobot help`).
- Run `cleobot doctor` to see the exact issues.
- Run `cleobot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `cleobot config set`. Keep a backup of `~/.openclaw/openclaw.json`.
...
CleoBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).

Additionally, it loads:

- `.env` from the current working directory (if present)
- a global fallback `.env` from `~/.openclaw/.env` (aka `$CLEOBOT_STATE_DIR/.env`)
...
Opt-in convenience: if enabled and none of the expected keys are set yet, CleoBot runs your login shell and imports only the missing expected keys (never overrides).
...
CleoBot stores **per-agent** auth profiles (OAuth + API keys) in:
...
- `~/.openclaw/credentials/oauth.json` (or `$CLEOBOT_STATE_DIR/credentials/oauth.json`)
...
- `~/.openclaw/agent/*` (migrated by `cleobot doctor` into `~/.openclaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `CLEOBOT_OAUTH_DIR`
- Agent dir (default agent root override): `CLEOBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, CleoBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, CleoBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/openclaw/cleobot-YYYY-MM-DD.log`
...
- The legacy single-account Baileys auth dir is migrated by `cleobot doctor` into `whatsapp/default`.
...
CleoBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["cleobot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
CleoBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `cleobot plugins install @openclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
CleoBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
CleoBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, CleoBot
...
When a file exceeds this limit, CleoBot logs a warning and injects a truncated
...
message envelopes). If unset, CleoBot uses the host timezone at runtime.
...
`"[openclaw]"`, CleoBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, CleoBot generates audio
...
CleoBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, CleoBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `cleobot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in CleoBot tools (excludes provider plugins)
...
OpenCode Zen is a multi-model gateway with per-model endpoints. CleoBot uses
...
- Shortcut: `cleobot onboard --auth-choice opencode-zen`.
...
Shortcut: `cleobot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `cleobot onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `cleobot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `cleobot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `cleobot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CLEOBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, CleoBot stays in idle-only mode for backward compatibility.
...
### `browser` (cleobot-managed browser)

CleoBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for cleobot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (CleoBot.app menubar, or `cleobot gateway`).
...
- When a connection comes from one of these IPs, CleoBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `cleobot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `CLEOBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `CLEOBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. CleoBot
...
- CleoBot.app watches `~/.openclaw/openclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.openclaw/openclaw.json` (or `CLEOBOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.openclaw/openclaw.json` (or `CLEOBOT_CONFIG_PATH`)
...
- `CLEOBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `cleobot --dev …` → uses `~/.cleobot-dev` + shifts ports from base `19001`
- `cleobot --profile <name> …` → uses `~/.cleobot-<name>` (port via config/env/flags)
...
```bash
CLEOBOT_CONFIG_PATH=~/.openclaw/a.json \
...
Gmail helper config (used by `cleobot webhooks gmail setup` / `run`):
...
- Set `CLEOBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, CleoBot defaults `serve.path` to `/` so
...
Default port: `18793` (chosen to avoid the cleobot browser CDP port `18792`)  
...
- env: `CLEOBOT_SKIP_CANVAS_HOST=1`
...
Controls LAN mDNS discovery broadcasts (`_cleobot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `openclaw` (advertises `openclaw.local`). Override with `CLEOBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_cleobot-gw._tcp` under `~/.openclaw/dns/` using the configured discovery domain (example: `openclaw.internal.`).
