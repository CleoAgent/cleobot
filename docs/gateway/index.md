...
- Replaces the legacy `gateway` command. CLI entry point: `cleobot gateway`.
...
- Config hot reload watches `~/.openclaw/openclaw.json` (or `CLEOBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclaw__/canvas/` from `~/.openclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `CLEOBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `CLEOBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `CLEOBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- Linux: `cleobot-gateway-<profile>.service`
- Windows: `CleoBot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `CLEOBOT_SERVICE_MARKER=openclaw`
- `CLEOBOT_SERVICE_KIND=gateway`
- `CLEOBOT_SERVICE_VERSION=<version>`
...
- `CLEOBOT_STATE_DIR=~/.cleobot-dev`
- `CLEOBOT_CONFIG_PATH=~/.cleobot-dev/openclaw.json`
- `CLEOBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `CLEOBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `CLEOBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `CLEOBOT_CONFIG_PATH`
- unique `CLEOBOT_STATE_DIR`
...
```bash
CLEOBOT_CONFIG_PATH=~/.openclaw/a.json CLEOBOT_STATE_DIR=~/.cleobot-a cleobot gateway --port 19001
CLEOBOT_CONFIG_PATH=~/.openclaw/b.json CLEOBOT_STATE_DIR=~/.cleobot-b cleobot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `cleobot health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `cleobot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.openclaw.*` is cleaned up).
  - `cleobot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are CleoBot profile services.
  ...
  - Cleanup: `cleobot gateway uninstall` (current service) and `cleobot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `cleobot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- CleoBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.openclaw.*` labels still unload cleanly).
- To stop it cleanly, use `cleobot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `cleobot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `cleobot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

CleoBot installs a **systemd user service** by default on Linux/WSL2. We
...
`cleobot gateway install` writes the user unit. `cleobot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/cleobot-gateway[-<profile>].service`:

```
[Unit]
Description=CleoBot Gateway (profile: <profile>, v<version>)
...
Restart=always
RestartSec=5
Environment=CLEOBOT_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/cleobot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now cleobot-gateway[-<profile>].service
```
...
- `cleobot gateway health|status` — request health/status over the Gateway WS.
- `cleobot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `cleobot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `cleobot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `cleobot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `cleobot gateway` and the legacy TCP control port.
