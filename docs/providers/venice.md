---
summary: "Use Venice AI privacy-focused models in CleoBot"
read_when:
  - You want privacy-focused inference in CleoBot
...
After setup, CleoBot shows all available Venice models. Pick based on your needs:
...
## Configure via `cleobot configure`

1. Run `cleobot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
CleoBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `cleobot models list` to see currently available models. Some models may be temporarily offline.
