...
CleoBot features that can generate provider usage or paid API calls.
...
- `cleobot status --usage` and `cleobot channels list` show provider **usage windows**
...
CleoBot can pick up credentials from:
...
- `cleobot status --usage`
- `cleobot models status --json`
...
`cleobot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
