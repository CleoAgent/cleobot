...
This document explains how CleoBot manages sessions end-to-end:
...
CleoBot is designed around a single **Gateway process** that owns session state.
...
CleoBot persists sessions in two layers:
...
- Transcripts: `~/.cleobot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

CleoBot resolves these via `src/config/sessions.ts`.
...
CleoBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (CleoBot consumes the events, but Pi decides when to compact).
...
CleoBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, CleoBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, CleoBot leaves it alone.
...
- `cleobot status` (CLI)
- `cleobot sessions` / `sessions --json`
...
CleoBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- CleoBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, CleoBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
CleoBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but CleoBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `cleobot status`.
