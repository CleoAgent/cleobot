...
3. Set tokens for CleoBot and start the gateway.
...
CleoBot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, CleoBot falls back to the user token.
...
```json
{
  "display_information": {
    "name": "CleoBot",
    "description": "Slack connector for CleoBot"
  },
  "features": {
    "bot_user": {
      "display_name": "CleoBot",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to CleoBot",
        "should_escape": false
      }
    ]
  },
...
By default, CleoBot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, CleoBot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `cleobot pairing approve slack <code>`.
...
- On startup, CleoBot resolves channel/user names in allowlists to IDs (when tokens allow)
