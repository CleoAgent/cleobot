...
CleoBot will offer the local install path automatically.
...
3. Configure CleoBot with those credentials.
...
- Talk to CleoBot via Teams DMs, group chats, or channels.
...
- On startup, CleoBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring CleoBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `cleobot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `cleobot plugins install @openclaw/msteams`
   - From a local checkout: `cleobot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "CleoBot in Teams", "full": "CleoBot in Teams" },
...
CleoBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, CleoBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/CleoBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

CleoBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `cleobot message poll --channel msteams --target conversation:<id> ...`
