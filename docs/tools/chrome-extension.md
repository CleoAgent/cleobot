---
summary: "Chrome extension: let CleoBot drive your existing Chrome tab"
read_when:
...
The CleoBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate cleobot-managed Chrome profile.
...
CleoBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the CleoBot release (npm package) as static files. There is no separate “build” step.

After upgrading CleoBot:

- Re-run `cleobot browser extension install` to refresh the installed files under your CleoBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

CleoBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want CleoBot to control.
...
- `ON`: attached; CleoBot can drive that tab.
...
Debugging: `cleobot sandbox explain`
...
`cleobot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `cleobot browser extension install` first to copy the extension to a stable location under your CleoBot state directory.
...
- **This is not isolated** like the dedicated cleobot-managed profile.
