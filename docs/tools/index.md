---
summary: "Agent tool surface for CleoBot (browser, canvas, nodes, message, cron) replacing legacy `cleobot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `cleobot-*` skills
title: "Tools"
---

# Tools (CleoBot)

CleoBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `cleobot-*` skills: the tools are typed, no shelling,
...
- If `tools.allow` only references unknown or unloaded plugin tool names, CleoBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`cleobot node run`).
...
- Requires a Brave API key (recommended: `cleobot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated CleoBot-managed browser.
...
- Quick smoke: `cleobot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `cleobot gateway` restart in-place)
