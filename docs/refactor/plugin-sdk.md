...
Accessed via `CleoBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: CleoBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: CleoBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: CleoBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: CleoBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: CleoBotConfig): string;
  };
};
```
...
- Add `api.runtime` to `CleoBotPluginApi` with the surface above.
