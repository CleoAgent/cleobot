...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.cleobot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.cleobot/node.json` (node id + pairing token).
...
Path: `~/.cleobot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.cleobot/exec-approvals.sock",
...
- Unix socket at `~/.cleobot/exec-approvals.sock` (0600).
