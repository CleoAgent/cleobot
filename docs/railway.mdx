...
Deploy CleoBot on Railway with a one-click template and finish setup in your browser.
...
- `CLEOBOT_STATE_DIR=/data/.openclaw` (recommended)
- `CLEOBOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `CLEOBOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your CleoBot state + workspace so you can migrate to another host without losing config or memory.
