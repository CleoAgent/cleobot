...
<p align="center">
  ...
  <a href="/">文档</a> ·
  <a href="/start/openclaw">CleoBot 助手设置</a>
</p>

CleoBot 将 WhatsApp（通过 WhatsApp Web / Baileys）、Telegram（Bot API / grammY）、Discord（Bot API / channels.discord.js）和 iMessage（imsg CLI）桥接至编程智能体，例如 [Pi](https://github.com/badlogic/pi-mono)。插件可添加 Mattermost（Bot API + WebSocket）等更多渠道支持。
CleoBot 同时也驱动着 CleoBot 助手。

## 从这里开始

- **从零开始全新安装：** [入门指南](/start/getting-started)
- **引导式设置（推荐）：** [向导](/start/wizard) （`cleobot onboard`）
- **打开仪表盘（本地 Gateway网关）：** http://127.0.0.1:18789/（或 http://localhost:18789/）

如果 Gateway网关运行在这台计算机上，该链接会立即打开浏览器控制界面。如果无法打开，请先启动 Gateway网关： `cleobot gateway`.
...
<p align="center">
  <img src="/whatsapp-openclaw.jpg" alt="CleoBot（英文原图）" width="360" />
  <img src="/whatsapp-cleobot-ai-zh.jpg" alt="CleoBot（AI 自动翻译）" width="360" />
  <br />
...
大多数操作通过 **Gateway网关** （`cleobot gateway`进行，它是一个长期运行的单进程，负责管理渠道连接和 WebSocket 控制面。
...
- **优先 local loopback**：Gateway网关 WS 默认监听 `ws://127.0.0.1:18789`.
  ...
  - 如需 Tailnet 访问，请运行 `cleobot gateway --bind tailnet --token ...` （非 local loopback 绑定时必须提供令牌）。
...
之后在 npm 安装和 git 安装之间切换很简单：安装另一种方式并运行 `cleobot doctor` 以更新 Gateway网关服务入口点。
...
如果尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm cleobot ...` （在仓库目录中执行）。
...
配置文件位于 `~/.cleobot/openclaw.json`.

- 如果你 **不做任何操作**，CleoBot 将使用内置的 Pi 二进制文件以 RPC 模式运行，并采用按发送者区分的会话。
...
**CleoBot = CLAW + TARDIS** — 因为每只太空龙虾都需要一台时空机器。
