...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.cleobot-dev`，并将 Gateway网关默认端口设为 `19001`（派生端口随之偏移）。
...
如果你还没有全局安装，可通过 `pnpm cleobot ...` 运行 CLI。
...
1. **配置文件隔离**（全局 `--dev`）
   ...
   - `CLEOBOT_GATEWAY_PORT=19001`（浏览器/画布端口随之偏移）
...
2. **开发引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（礼仪机器人）。
   - 在开发模式下跳过渠道提供商（`CLEOBOT_SKIP_CHANNELS=1`）。
