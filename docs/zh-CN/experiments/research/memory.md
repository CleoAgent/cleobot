---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.cleobot/workspace）
  - 决策：独立 CLI 还是深度集成 CleoBot
...
目标：Clawd 风格的工作区（`agents.defaults.workspace`，默认 `~/.cleobot/workspace`），其中"记忆"以每日一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（如 `memory.md`、`SOUL.md`）的形式存储。
...
保持 `~/.cleobot/workspace` 作为规范的人类可读记忆。
...
```
~/.cleobot/workspace/.memory/index.sqlite
```
...
- CleoBot 已经知道：
  ...
  - 日志 + 故障排除模式
- 你希望智能体自身调用这些工具：
  - `cleobot memory recall "…" --k 25 --since 30d`
  - `cleobot memory reflect --since 7d`
