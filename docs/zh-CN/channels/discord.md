...
- 通过 Discord 私信或服务器频道与 CleoBot 对话。
...
3. 使用 `channels.discord.token` 配置 CleoBot（或使用 `DISCORD_BOT_TOKEN` 作为回退）。
...
7. 私聊：默认通过 `channels.discord.dm.policy`（默认：`"pairing"`）进行安全保护。未知发送者会收到配对码（1 小时后过期）；通过 `cleobot pairing approve discord <code>` 批准。
...
这是在服务器（guild）频道（如 `#help`）中运行 CleoBot 的"Discord 开发者门户"设置。
...
Discord 到处使用数字 ID；CleoBot 配置推荐使用 ID。
...
- 首先：运行 `cleobot doctor` 和 `cleobot channels status --probe`（可操作的警告 + 快速审计）。
...
启用 PK 查找，使代理消息解析为底层的系统 + 成员。启用后，CleoBot 使用成员身份进行允许列表匹配，并将发送者标记为 `Member (PK:System)` 以避免意外的 Discord ping。
...
- 启动时，CleoBot 将允许列表中的频道/用户名称解析为 ID（当机器人可以搜索成员时）并记录映射；未解析的条目保持原样。

原生命令注意事项：

- 注册的命令与 CleoBot 的聊天命令一致。
- 原生命令遵循与私信/服务器消息相同的允许列表（`channels.discord.dm.allowFrom`、`channels.discord.guilds`、按频道规则）。
- 斜杠命令在 Discord UI 中可能对不在允许列表中的用户仍然可见；CleoBot 在执行时强制执行允许列表并回复"未授权"。
...
- 如果机器人卡住或被速率限制，在确认没有其他进程占用 Discord 会话后重启 Gateway网关（`cleobot gateway --force`）。
