...
- 通过 CLI 安装：`cleobot plugins install @openclaw/zalo`
...
1. 安装 Zalo 插件：
   - 从源码检出安装：`cleobot plugins install ./extensions/zalo`
   - 从 npm 安装（如已发布）：`cleobot plugins install @openclaw/zalo`
   - 或在新手引导中选择 **Zalo** 并确认安装提示
2. 设置令牌：
   - 环境变量：`ZALO_BOT_TOKEN=...`
...
- 使用聊天 ID 作为目标。
- 示例：`cleobot message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**机器人无响应：**

- 检查令牌是否有效：`cleobot channels status --probe`
- 验证发送者是否已批准（配对或 allowFrom）
- 检查 Gateway网关日志：`cleobot logs --follow`
