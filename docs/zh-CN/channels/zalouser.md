...
- 通过 CLI 安装：`cleobot plugins install @openclaw/zalouser`
- 或从源码检出安装：`cleobot plugins install ./extensions/zalouser`
- 详情：[插件](/plugin)
...
- 配置向导可以提示设置群组允许列表。
- 启动时，CleoBot 会将允许列表中的群组/用户名称解析为 ID 并记录映射关系；未解析的条目保持原样。
...
- 重新登录：`cleobot channels logout --channel zalouser && cleobot channels login --channel zalouser`
