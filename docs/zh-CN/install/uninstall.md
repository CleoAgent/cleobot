...
read_when:
  - 你想从机器上移除 CleoBot
  - 卸载后 Gateway网关服务仍在运行
summary: 完全卸载 CleoBot（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
如果你将 `CLEOBOT_CONFIG_PATH` 设置为状态目录之外的自定义位置，请同时删除该文件。
...
- 如果你使用了配置文件（`--profile` / `CLEOBOT_PROFILE`），请对每个状态目录重复步骤 3（默认为 `~/.cleobot-<profile>`）。
...
默认单元名称为 `cleobot-gateway.service`（或 `cleobot-gateway-<profile>.service`）：
...
默认任务名称为 `CleoBot Gateway网关`（或 `CleoBot Gateway网关 (<profile>)`）。
任务脚本位于你的状态目录下。

```powershell
schtasks /Delete /F /TN "CleoBot Gateway网关"
Remove-Item -Force "$env:USERPROFILE\.openclaw\gateway.cmd"
```

如果你使用了配置文件，请删除对应的任务名称和 `~\.cleobot-<profile>\gateway.cmd`。
...
如果你从仓库检出运行（`git clone` + `cleobot ...` / `bun run cleobot ...`）：
