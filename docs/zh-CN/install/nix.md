...
使用 Nix 运行 CleoBot 的推荐方式是通过 **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** — 一个开箱即用的 Home Manager 模块。
...
当设置了 `CLEOBOT_NIX_MODE=1` 时（nix-openclaw 会自动设置）：

CleoBot 支持 **Nix 模式**，使配置具有确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
CleoBot 从 `CLEOBOT_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `CLEOBOT_STATE_DIR` 中。

- `CLEOBOT_STATE_DIR`（默认：`~/.cleobot`）
- `CLEOBOT_CONFIG_PATH`（默认：`$CLEOBOT_STATE_DIR/openclaw.json`）
