...
read_when:
  - 更新 CleoBot
  - 更新后出现问题
summary: 安全更新 CleoBot（全局安装或源码安装），以及回滚策略
title: 更新
x-i18n:
...
CleoBot 迭代速度很快（尚未到"1.0"）。请像对待基础设施发布一样对待更新：更新 → 运行检查 → 重启（或使用 `cleobot update`，它会自动重启）→ 验证。

## 推荐方式：重新运行网站安装程序（原地升级）

**首选**更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `cleobot doctor`。
...
- 备份你的自定义配置：
  - 配置文件：`~/.cleobot/openclaw.json`
  - 凭据：`~/.cleobot/credentials/`
  - 工作区：`~/.cleobot/workspace`
...
- 如果你的 Gateway网关作为服务运行，建议使用 `cleobot gateway restart` 而非直接终止 PID。
...
- 切换到所选渠道（标签或分支）。
- 从配置的上游（dev 渠道）拉取并变基。
- 安装依赖、构建、构建控制面板 UI，并运行 `cleobot doctor`。
- 默认重启 Gateway网关（使用 `--no-restart` 跳过）。

如果你通过 **npm/pnpm** 安装（无 git 元数据），`cleobot update` 会尝试通过你的包管理器进行更新。如果无法检测到安装方式，请改用"更新（全局安装）"。

## 更新（控制面板 UI / RPC）

控制面板 UI 提供**更新并重启**功能（RPC：`update.run`）。它会：

1. 执行与 `cleobot update` 相同的源码更新流程（仅限 git checkout）。
...
- 如果你从仓库检出运行而没有全局安装，请使用 `pnpm cleobot ...` 执行 CLI 命令。
- 如果你直接从 TypeScript 运行（`pnpm cleobot ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
- 在全局安装和 git 安装之间切换很容易：安装另一种方式，然后运行 `cleobot doctor`，这样 Gateway网关服务入口点会被重写为当前安装。

## 必须执行：`cleobot doctor`

Doctor 是"安全更新"命令。它有意设计得很朴素：修复 + 迁移 + 警告。

注意：如果你使用的是**源码安装**（git checkout），`cleobot doctor` 会建议先运行 `cleobot update`。
...
- 检测并将旧版 Gateway网关服务（launchd/systemd；旧版 schtasks）迁移到当前的 CleoBot 服务。
...
- Linux systemd 用户服务：`systemctl --user restart cleobot-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart cleobot-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则请运行 `cleobot gateway install`。
...
提示：要查看当前已发布的版本，请运行 `npm view cleobot version`。
...
- 再次运行 `cleobot doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
