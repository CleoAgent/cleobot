...
"上下文"是 **CleoBot 在一次运行中发送给模型的所有内容**。它受模型的**上下文窗口**（token 限制）约束。

初学者心智模型：

- **系统提示词**（由 CleoBot 构建）：规则、工具、Skills 列表、时间/运行时信息，以及注入的工作区文件。
...
系统提示词由 **CleoBot 拥有**，每次运行时重新构建。它包括：
...
默认情况下，CleoBot 会注入一组固定的工作区文件（如果存在）：
