...
- CLI 辅助命令：`cleobot onboard`、`cleobot models list`、`cleobot models set <provider/model>`。

## 内置提供商（pi-ai 目录）

CleoBot 自带 pi‑ai 目录。这些提供商**无需**配置 `models.providers`；只需设置认证并选择模型即可。
...
- CLI：`cleobot onboard --auth-choice token`（粘贴 setup-token）或 `cleobot models auth paste-token --provider anthropic`
...
- CLI：`cleobot onboard --auth-choice openai-codex` 或 `cleobot models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`cleobot plugins enable google-antigravity-auth`
  - 登录：`cleobot models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`cleobot plugins enable google-gemini-cli-auth`
  - 登录：`cleobot models auth login --provider google-gemini-cli --set-default`
