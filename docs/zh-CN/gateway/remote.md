...
- 使用 macOS 应用的**通过 SSH 远程连接**模式（设置 → 通用 → "CleoBot 运行位置"）。
...
- `cleobot health` 和 `cleobot status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway网关。
- `cleobot gateway {status,health,send,agent,call}` 也可以在需要时通过 `--url` 指向转发的 URL。

注意：将 `18789` 替换为你配置的 `gateway.port`（或 `--port`/`CLEOBOT_GATEWAY_PORT`）。
