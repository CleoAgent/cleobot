...
CleoBot 的 Gateway网关暴露了一个简单的 HTTP 端点，用于直接调用单个工具。该端点始终启用，但受 Gateway网关认证和工具策略控制。
...
- `x-cleobot-message-channel: <channel>`（示例：`slack`、`telegram`）
- `x-cleobot-account-id: <accountId>`（当存在多个账户时）
