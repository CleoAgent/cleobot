...
CleoBot 的 Gateway网关可以提供一个小型的兼容 OpenAI 的 Chat Completions 端点。
...
在底层，请求会作为普通的 Gateway网关智能体运行来执行（与 `cleobot agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway网关保持一致。
...
或通过请求头指定特定的 CleoBot 智能体：

- `x-cleobot-agent-id: <agentId>`（默认值：`main`）

高级用法：

- `x-cleobot-session-key: <sessionKey>` 用于完全控制会话路由。
