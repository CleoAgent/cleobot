...
- 替代旧版 `gateway` 命令。CLI 入口：`cleobot gateway`。
...
- 配置热重载会监视 `~/.cleobot/openclaw.json`（或 `CLEOBOT_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）启动 Canvas 文件服务器，从 `~/.cleobot/workspace/canvas` 提供 `http://<gateway-host>:18793/__openclaw__/canvas/` 服务。通过 `canvasHost.enabled=false` 或 `CLEOBOT_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway网关认证：设置 `gateway.auth.token`（或 `CLEOBOT_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 local loopback 上也是如此。
- 端口优先级：`--port` > `CLEOBOT_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- Windows：`CleoBot Gateway网关 (<profile>)`
...
```bash
CLEOBOT_CONFIG_PATH=~/.cleobot/a.json CLEOBOT_STATE_DIR=~/.cleobot-a cleobot gateway --port 19001
CLEOBOT_CONFIG_PATH=~/.cleobot/b.json CLEOBOT_STATE_DIR=~/.cleobot-b cleobot gateway --port 19002
```
...
- `health` — 完整健康快照（与 `cleobot health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，请使用自定义 LaunchDaemon（未随附）。
  - `cleobot gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.openclaw.*` 会被清理）。
  - `cleobot doctor` 审计 LaunchAgent 配置，并可将其更新为当前推荐的默认值。
...
- 如果检测到其他类似 Gateway网关的服务，CLI 会发出警告，除非它们是 CleoBot 配置文件服务。
  我们仍建议大多数场景下**每台机器一个 Gateway网关**；使用隔离的配置文件/端口实现冗余或救援机器人。参见 [多 Gateway网关](/gateway/multiple-gateways)。
  - 清理：`cleobot gateway uninstall`（当前服务）和 `cleobot doctor`（旧版迁移）。
- `gateway install` 在已安装时为空操作；使用 `cleobot gateway install --force` 重新安装（配置文件/环境/路径变更）。

捆绑的 Mac 应用：

- CleoBot.app 可以捆绑一个基于 Node 的 Gateway网关中继，并安装按用户的 LaunchAgent，标签为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.openclaw.*` 标签仍可正常卸载）。
- 要正常停止，使用 `cleobot gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `cleobot gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `cleobot gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 进程监管（systemd 用户单元）

CleoBot 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们推荐单用户机器使用用户服务（更简单的环境，按用户配置）。对于多用户或始终在线的服务器，使用**系统服务**（无需 lingering，共享监管）。

`cleobot gateway install` 写入用户单元。`cleobot doctor` 审计该单元，并可将其更新为当前推荐的默认值。
...
**替代方案（系统服务）** - 对于始终在线或多用户服务器，可以安装 systemd **系统**单元而非用户单元（无需 lingering）。创建 `/etc/systemd/system/cleobot-gateway[-<profile>].service`（复制上述单元，将 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：
...
- `cleobot gateway health|status` — 通过 Gateway网关 WS 请求健康/状态信息。
- `cleobot message send --target <num> --message "hi" [--media ...]` — 通过 Gateway网关发送（对 WhatsApp 具有幂等性）。
- `cleobot agent --message "hi" --to <num>` — 运行智能体回合（默认等待最终结果）。
- `cleobot gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `cleobot gateway stop|restart` — 停止/重启受监管的 Gateway网关服务（launchd/systemd）。
- Gateway网关辅助子命令假设 Gateway网关已在 `--url` 上运行；它们不再自动启动 Gateway网关。

## 迁移指南

- 弃用 `cleobot gateway` 和旧版 TCP 控制端口的用法。
