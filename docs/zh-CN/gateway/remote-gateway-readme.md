...
summary: 为 CleoBot.app 连接远程 Gateway网关设置 SSH 隧道
...
# 使用远程 Gateway网关运行 CleoBot.app

CleoBot.app 使用 SSH 隧道连接到远程 Gateway网关。本指南将介绍如何进行设置。
...
│                        客户端机器                              │
│                                                              │
│  CleoBot.app ──► ws://127.0.0.1:18789（本地端口）              │
│                     │                                        │
│                     ▼                                        │
│  SSH 隧道 ──────────────────────────────────────────────────│
│                     │                                        │
...
### 第 5 步：重启 CleoBot.app

```bash
# 退出 CleoBot.app（⌘Q），然后重新打开：
...
CleoBot.app 连接到客户端机器上的 `ws://127.0.0.1:18789`。SSH 隧道将该连接转发到运行 Gateway网关的远程机器的 18789 端口。
