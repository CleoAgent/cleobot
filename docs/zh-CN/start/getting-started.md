...
- **推荐：** Brave Search API 密钥用于网络搜索。最简单的方式：
  `cleobot configure --section web` （存储 `tools.web.search.apiKey`）。
  参见 [网络工具](/tools/web)。
...
- OAuth 凭据（旧版导入）： `~/.cleobot/credentials/oauth.json`
...
如果你尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm cleobot ...` （从仓库中）。
...
如果 `cleobot health` 显示"未配置认证"，请返回向导设置 OAuth/密钥认证——智能体在没有认证的情况下将无法响应。

提示： `cleobot status --all` 是最佳的可粘贴只读调试报告。
健康探针： `cleobot health` （或 `cleobot status --deep`）向运行中的 Gateway网关请求健康快照。
