...
summary: 设置指南：在保持更新的同时维护你的个性化 CleoBot 配置
title: 设置
x-i18n:
...
- **个性化配置存放在仓库之外：** `~/.cleobot/workspace`（工作区）+ `~/.cleobot/openclaw.json`（配置）。
...
- **配置：** `~/.cleobot/openclaw.json`（JSON/JSON5 风格）
- **工作区：** `~/.cleobot/workspace`（Skills、提示词、记忆；建议设为私有 git 仓库）
...
如果你还没有全局安装，请通过 `pnpm cleobot setup` 运行。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **CleoBot.app**（菜单栏）。
...
- 运行 `cleobot setup`，然后 `cleobot channels login`，接着手动启动 Gateway网关（`cleobot gateway`）。
...
### 2) 将 macOS 应用指向你运行的 Gateway网关在 **CleoBot.app** 中：
...
- **模型认证配置文件**：`~/.cleobot/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.cleobot/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的配置）

- 将 `~/.cleobot/workspace` 和 `~/.cleobot/` 视为"你的东西"；不要将个人提示词/配置放入 `openclaw` 仓库中。
