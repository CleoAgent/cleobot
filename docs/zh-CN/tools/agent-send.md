...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接通过 `cleobot agent` CLI 运行（支持可选的消息投递）
title: 智能体发送
x-i18n:
...
# `cleobot agent`（直接运行智能体）

`cleobot agent` 无需入站聊天消息即可运行单次智能体轮次。
...
- 输出：
  ...
  - `--json`：打印结构化载荷 + 元数据
- 可选通过 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `cleobot message --target` 一致）。
