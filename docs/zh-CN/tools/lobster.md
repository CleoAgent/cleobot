...
summary: CleoBot 的类型化工作流运行时，支持可恢复的审批门控。
title: Lobster
x-i18n:
...
Lobster 是一个工作流外壳，让 CleoBot 能够将多步骤工具序列作为单个确定性操作运行，并带有显式审批检查点。
...
- **一次调用代替多次**：CleoBot 运行一次 Lobster 工具调用即可获得结构化结果。
...
CleoBot 以**工具模式**启动本地 `lobster` CLI，并从 stdout 解析 JSON 信封。
...
Lobster 可以运行包含 `name`、`args`、`steps`、`env`、`condition` 和 `approval` 字段的 YAML/JSON 工作流文件。在 CleoBot 工具调用中，将 `pipeline` 设置为文件路径。
...
在运行 CleoBot Gateway网关的**同一主机**上安装 Lobster CLI（参见 [Lobster 仓库](https://github.com/openclaw/lobster)），并确保 `lobster` 在 `PATH` 中。
...
插件工具（如 `lobster`），CleoBot 会保持核心工具启用。要限制核心
...
```
用户："检查我的邮件并起草回复"
→ cleobot 调用 gmail.list
→ LLM 进行摘要
→ 用户："给 #2 和 #5 起草回复"
→ LLM 起草
→ 用户："发送 #2"
→ cleobot 调用 gmail.send
（每天重复，不记得之前分类了什么）
```
...
- **仅限本地子进程** — 插件本身不发起网络调用。
- **无密钥** — Lobster 不管理 OAuth；它调用处理 OAuth 的 CleoBot 工具。
