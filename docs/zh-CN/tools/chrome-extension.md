...
read_when:
  ...
  - 你想了解浏览器接管的安全影响
summary: Chrome 扩展：让 CleoBot 控制你现有的 Chrome 标签页
title: Chrome 扩展
x-i18n:
...
CleoBot Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你日常使用的 Chrome 窗口），而不是启动一个单独的 CleoBot 托管 Chrome 配置文件。
...
CleoBot 随后通过常规的 `browser` 工具界面（选择正确的配置文件）控制附加的标签页。
...
扩展作为静态文件包含在 CleoBot 发行版（npm 包）中。没有单独的"构建"步骤。

升级 CleoBot 后：

- 重新运行 `cleobot browser extension install` 以刷新 CleoBot 状态目录下的已安装文件。
- Chrome → `chrome://extensions` → 点击扩展上的"重新加载"。

## 使用方法（无需额外配置）

CleoBot 内置了一个名为 `chrome` 的浏览器配置文件，指向默认端口上的扩展中继。
...
`cleobot browser extension path` 打印包含扩展文件的**已安装**磁盘目录。

CLI 故意**不会**打印 `node_modules` 路径。请始终先运行 `cleobot browser extension install`，将扩展复制到 CleoBot 状态目录下的稳定位置。
