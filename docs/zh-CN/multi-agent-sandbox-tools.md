...
有关调试"为什么被阻止了？"，请参阅[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated)和 `cleobot sandbox explain`。
...
旧版 `agent.*` 配置会由 `cleobot doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
...
1. **检查智能体路由解析：**
   ...
   ```exec
   cleobot agents list --bindings
   ```

2. **验证沙箱容器：**

   ```exec
   docker ps --filter "name=cleobot-sbx-"
   ```
...
4. **监控日志：**
   ```exec
   tail -f "${CLEOBOT_STATE_DIR:-$HOME/.openclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
