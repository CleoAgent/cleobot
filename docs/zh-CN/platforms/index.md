...
CleoBot 核心使用 TypeScript 编写。**推荐使用 Node 作为运行时**。
...
- 服务状态：`cleobot gateway status`

## Gateway网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`cleobot onboard --install-daemon`
- 直接安装：`cleobot gateway install`
- 配置流程：`cleobot configure` → 选择 **Gateway网关 service**
- 修复/迁移：`cleobot doctor`（会提示安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.openclaw.*`）
- Linux/WSL2：systemd 用户服务（`cleobot-gateway[-<profile>].service`）
