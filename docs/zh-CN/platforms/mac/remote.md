...
read_when:
  - 设置或调试远程 Mac 控制
summary: macOS 应用通过 SSH 远程控制 CleoBot Gateway网关的流程
title: 远程控制
x-i18n:
...
# 远程 CleoBot（macOS ⇄ 远程主机）

此流程让 macOS 应用充当运行在另一台主机（桌面/服务器）上的 CleoBot Gateway网关的完整远程控制器。这是应用的 **Remote over SSH**（远程运行）功能。所有功能——健康检查、语音唤醒转发和 Web Chat——都复用 _设置 → 通用_ 中相同的远程 SSH 配置。

## 模式

- **本地（此 Mac）**：所有内容在笔记本电脑上运行，无需 SSH。
- **Remote over SSH（默认）**：CleoBot 命令在远程主机上执行。Mac 应用使用 `-o BatchMode` 加上你选择的身份/密钥打开 SSH 连接，并进行本地端口转发。
...
1. 安装 Node + pnpm 并构建/安装 CleoBot CLI（`pnpm install && pnpm build && pnpm link --global`）。
...
1. 打开 _设置 → 通用_。
2. 在 **CleoBot runs** 下，选择 **Remote over SSH** 并设置：
...
3. 点击 **测试远程连接**。成功表示远程 `cleobot status --json` 正常运行。失败通常意味着 PATH/CLI 问题；exit 127 表示远程未找到 CLI。
...
- 在**远程主机**上运行 `cleobot channels login --verbose`。用手机上的 WhatsApp 扫描二维码。
...
- **健康探测失败**：检查 SSH 可达性、PATH，以及 Baileys 是否已登录（`cleobot status --json`）。
