...
- 应用每约 60 秒以及按需通过 `ShellExecutor` 运行 `cleobot health --json`。探测会加载凭据并报告状态，不会发送消息。
- 分别缓存上次成功的快照和上次错误，以避免闪烁；显示各自的时间戳。

## 不确定时

- 你仍然可以使用 [Gateway网关健康检查](/gateway/health) 中的 CLI 流程（`cleobot status`、`cleobot status --deep`、`cleobot health --json`），并跟踪 `/tmp/openclaw/cleobot-*.log` 中的 `web-heartbeat` / `web-reconnect`。
