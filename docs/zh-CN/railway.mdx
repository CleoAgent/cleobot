...
通过一键模板在 Railway 上部署 CleoBot，并在浏览器中完成设置。
...
- `CLEOBOT_STATE_DIR=/data/.openclaw`（推荐）
- `CLEOBOT_WORKSPACE_DIR=/data/workspace`（推荐）
- `CLEOBOT_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 CleoBot 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
