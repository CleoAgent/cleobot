...
CleoBot 围绕单个 **Gateway网关进程**设计，该进程拥有会话状态。
...
- 存储：`~/.cleobot/agents/<agentId>/sessions/sessions.json`
- 对话记录：`~/.cleobot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram 话题会话：`.../<sessionId>-topic-<threadId>.jsonl`

CleoBot 通过 `src/config/sessions.ts` 解析这些路径。
...
CleoBot 有意**不**"修复"对话记录；Gateway网关使用 `SessionManager` 来读写它们。
...
这些是 Pi 运行时语义（CleoBot 消费事件，但由 Pi 决定何时压缩）。
...
CleoBot 还为嵌入式运行强制执行安全下限：

- 如果 `compaction.reserveTokens < reserveTokensFloor`，CleoBot 会将其提升。
- 默认下限为 `20000` token。
...
CleoBot 支持用于后台任务的"静默"轮次，用户不应看到中间输出。

约定：

- 助手以 `NO_REPLY` 开始其输出，表示"不要向用户传递回复"。
- CleoBot 在传递层中剥离/抑制此标记。

自 `2026.1.10` 起，当部分块以 `NO_REPLY` 开头时，CleoBot 还会抑制**草稿/输入中的流式传输**，因此静默操作不会在轮次中途泄露部分输出。
...
Pi 也在扩展 API 中暴露了 `session_before_compact` 钩子，但 CleoBot 的刷写逻辑目前在 Gateway网关侧。
...
- 存储与对话记录不匹配？确认 Gateway网关主机以及 `cleobot status` 中的存储路径。
- 压缩过于频繁？检查：
  - 模型上下文窗口（太小）
