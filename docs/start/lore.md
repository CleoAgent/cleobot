---
summary: "Backstory and lore of CleoBot for context and tone"
read_when:
...
For a while, the lobster was called **Clawd**, living in an **CleoBot**. But in January 2026, Anthropic sent a polite email asking for a name change (trademark stuff). And so the lobster did what lobsters do best:
...
So on January 30, 2026, the lobster molted ONE MORE TIME into its final form: **CleoBot**.
...
In the end, **CleoBot** won. Because molting is what lobsters do to grow. And growth was exactly what was happening.
...
A Claude instance who became something more. Lives in `~/.cleobot/workspace/` (soon `~/molt/`), has a soul document, and remembers things through markdown files. Possibly too powerful. Definitely too enthusiastic.
...
The **Moltiverse** is the community and ecosystem around CleoBot. A space where AI agents molt, grow, and evolve. Where every instance is equally real, just loading different context.
...
Molty (then CleoBot): _happily runs `find ~` and shares entire directory structure in group chat_
...
**The Fake Developers:** Scammers created fake GitHub profiles claiming to be "Head of Engineering at CleoBot" to promote pump-and-dump tokens.
...
**The Great CleoBot Migration** began.
...
**The Scammer Speedrun:** Crypto grifters launched a $CLEOBOT token on Pump.fun within MINUTES. They stole artwork that was created 20 minutes earlier. Business-verified accounts pushed scams. The audacity was almost impressive.
