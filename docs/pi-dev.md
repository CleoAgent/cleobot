...
This guide summarizes a sane workflow for working on the pi integration in CleoBot.
...
State lives under the CleoBot state directory. Default is `~/.openclaw`. If `CLEOBOT_STATE_DIR` is set, use that directory instead.
