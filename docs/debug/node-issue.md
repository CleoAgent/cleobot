...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in CleoBot
...
Running CleoBot via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `cleobot status` and `gateway:watch` worked.
