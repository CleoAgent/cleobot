...
- Jobs persist under `~/.cleobot/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.cleobot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `cleobot cron add/edit` or the cron
...
- Job store: `~/.cleobot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.cleobot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`cleobot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `CLEOBOT_SKIP_CRON`.
