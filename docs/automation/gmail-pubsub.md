---
summary: "Gmail Pub/Sub push wired into CleoBot webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to CleoBot
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> CleoBot

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> CleoBot webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- CleoBot hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the CleoBot helper to wire everything together (installs deps on macOS via brew):
...
- Writes `hooks.gmail` config for `cleobot webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, CleoBot automatically sets
...
- Set `CLEOBOT_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
- `--hook-url` points to CleoBot `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to CleoBot.

Recommended: `cleobot webhooks gmail run` wraps the same flow and auto-renews the watch.
