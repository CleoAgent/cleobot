...
**Goal:** CleoBot Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
| `internal_port = 3000`         | Must match `--port 3000` (or `CLEOBOT_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `CLEOBOT_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
- Non-loopback binds (`--bind lan`) require `CLEOBOT_GATEWAY_TOKEN` for security.
...
**Note:** With `CLEOBOT_STATE_DIR=/data`, the config path is `/data/openclaw.json`.
...
Paste your gateway token (the one from `CLEOBOT_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `CLEOBOT_GATEWAY_PORT=3000`).
...
**Fix:** Ensure `CLEOBOT_STATE_DIR=/data` is set in `fly.toml` and redeploy.
