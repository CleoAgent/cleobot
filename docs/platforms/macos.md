---
summary: "CleoBot macOS companion app (menu bar + gateway broker)"
read_when:
...
# CleoBot macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for CleoBot. It owns permissions,
...
- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `cleobot gateway install`.
...
(or `bot.molt.<profile>` when using `--profile`/`CLEOBOT_PROFILE`; legacy `com.openclaw.*` still unloads).
...
1. Install and launch **CleoBot.app**.
...
Tip: compare against `cleobot gateway discover --json` to see whether the
