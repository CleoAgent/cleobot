...
- **Task tool** in the upstream spec == CleoBot `sessions_spawn`
- **File I/O** == CleoBot `read`/`write`
- **Remote fetch** == CleoBot `web_fetch` (or `exec` with curl when POST is required)
