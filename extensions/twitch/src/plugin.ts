/**
 * Twitch channel plugin for CleoBot.
 *
 ...
 * for CleoBot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: CleoBotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: CleoBotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: CleoBotConfig): boolean => {
