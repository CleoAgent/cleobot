...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CLEOBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-nostr-"));
  process.env.CLEOBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.CLEOBOT_STATE_DIR?.trim() || env.CLEOBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CLEOBOT_STATE_DIR = previous;
    }
