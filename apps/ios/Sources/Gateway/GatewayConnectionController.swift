...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [CleoBotCapability.canvas.rawValue, CleoBotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(CleoBotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(CleoBotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = CleoBotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(CleoBotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            CleoBotSystemCommand.notify.rawValue,
            CleoBotSystemCommand.which.rawValue,
            CleoBotSystemCommand.run.rawValue,
            CleoBotSystemCommand.execApprovalsGet.rawValue,
            CleoBotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(CleoBotCapability.camera.rawValue) {
            commands.append(CleoBotCameraCommand.list.rawValue)
            commands.append(CleoBotCameraCommand.snap.rawValue)
            commands.append(CleoBotCameraCommand.clip.rawValue)
        }
        if caps.contains(CleoBotCapability.location.rawValue) {
            commands.append(CleoBotLocationCommand.get.rawValue)
        }
