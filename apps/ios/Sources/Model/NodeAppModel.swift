...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = CleoBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = CleoBotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = CleoBotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = CleoBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = CleoBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__cleobot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: CleoBotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for cleobot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case CleoBotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case CleoBotCanvasCommand.present.rawValue,
                 CleoBotCanvasCommand.hide.rawValue,
                 CleoBotCanvasCommand.navigate.rawValue,
                 CleoBotCanvasCommand.evalJS.rawValue,
                 CleoBotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case CleoBotCanvasA2UICommand.reset.rawValue,
                 CleoBotCanvasA2UICommand.push.rawValue,
                 CleoBotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case CleoBotCameraCommand.list.rawValue,
                 CleoBotCameraCommand.snap.rawValue,
                 CleoBotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case CleoBotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: CleoBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CleoBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(CleoBotLocationGetParams.self, from: req.paramsJSON)) ??
            CleoBotLocationGetParams()
        ...
        let payload = CleoBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CleoBotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(CleoBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                CleoBotCanvasPresentParams()
        ...
        case CleoBotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CleoBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(CleoBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CleoBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(CleoBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CleoBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(CleoBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CleoBotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing cleobotA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CleoBotCanvasA2UICommand.push.rawValue, CleoBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == CleoBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(CleoBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try CleoBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(CleoBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try CleoBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.cleobotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CleoBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CleoBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(CleoBotCameraSnapParams.self, from: req.paramsJSON)) ??
                CleoBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CleoBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(CleoBotCameraClipParams.self, from: req.paramsJSON)) ??
                CleoBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(CleoBotScreenRecordParams.self, from: req.paramsJSON)) ??
            CleoBotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> CleoBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return CleoBotLocationMode(rawValue: raw) ?? .off
    }
