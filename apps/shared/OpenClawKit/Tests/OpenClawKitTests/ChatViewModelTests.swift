...
private final class TestChatTransport: @unchecked Sendable, CleoBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CleoBotChatHistoryPayload]
    private let sessionsResponses: [CleoBotChatSessionsListResponse]

    private let stream: AsyncStream<CleoBotChatTransportEvent>
    private let continuation: AsyncStream<CleoBotChatTransportEvent>.Continuation

    init(
        historyResponses: [CleoBotChatHistoryPayload],
        sessionsResponses: [CleoBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<CleoBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CleoBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CleoBotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [CleoBotChatAttachmentPayload]) async throws -> CleoBotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return CleoBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CleoBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CleoBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CleoBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = CleoBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = CleoBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { CleoBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = CleoBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = CleoBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = CleoBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { CleoBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
