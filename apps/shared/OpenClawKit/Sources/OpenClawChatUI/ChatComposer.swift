...
@MainActor
struct CleoBotChatComposer: View {
    @Bindable var viewModel: CleoBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(CleoBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(CleoBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \CleoBotPendingAttachment.id)
                { (att: CleoBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            CleoBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CleoBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(CleoBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(CleoBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
