...
private let chatUILogger = Logger(subsystem: "ai.cleobot", category: "CleoBotChatUI")

@MainActor
@Observable
public final class CleoBotChatViewModel {
    public private(set) var messages: [CleoBotChatMessage] = []
    ...
    public var attachments: [CleoBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CleoBotChatPendingToolCall] = []
    public private(set) var sessions: [CleoBotChatSessionEntry] = []
    private let transport: any CleoBotChatTransport
    ...
    private var pendingToolCallsById: [String: CleoBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any CleoBotChatTransport) {
    ...
    public var sessionChoices: [CleoBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [CleoBotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [CleoBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CleoBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CleoBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CleoBotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [CleoBotChatMessage]) -> [CleoBotChatMessage] {
        var result: [CleoBotChatMessage] = []
    ...
    private static func dedupeKey(for message: CleoBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CleoBotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> CleoBotChatAttachmentPayload in
            CleoBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CleoBotChatSessionEntry {
        CleoBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: CleoBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CleoBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CleoBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CleoBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> CleoBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
