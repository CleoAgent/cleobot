...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: CleoBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CleoBotProtocol.AnyCodable] = [
            "presence": .init([CleoBotProtocol.AnyCodable(entry)]),
        ]
        let payload = CleoBotProtocol.AnyCodable(payloadMap)
