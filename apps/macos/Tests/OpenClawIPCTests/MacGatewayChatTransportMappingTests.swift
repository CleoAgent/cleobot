...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: CleoBotProtocol.AnyCodable(["ok": CleoBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CleoBotProtocol.AnyCodable(["ok": CleoBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = CleoBotProtocol.AnyCodable([
            "runId": CleoBotProtocol.AnyCodable("run-1"),
            "sessionKey": CleoBotProtocol.AnyCodable("main"),
            "state": CleoBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CleoBotProtocol.AnyCodable(["a": CleoBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
