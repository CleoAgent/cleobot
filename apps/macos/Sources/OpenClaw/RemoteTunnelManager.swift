...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.cleobot", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous CleoBot run already has an SSH listener on the expected port (common after restarts),
