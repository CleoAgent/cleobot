...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.cleobot/cleobot.json. Prefer CLEOBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.cleobot/cleobot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.cleobot/cleobot.json. Restart the gateway to apply."
        }
