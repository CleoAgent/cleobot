...
private let webChatSwiftLogger = Logger(subsystem: "ai.cleobot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: CleoBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CleoBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(CleoBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CleoBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CleoBotGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CleoBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CleoBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CleoBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: CleoBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CleoBotChatView>) -> NSViewController
    {
