...
private let deepLinkLogger = Logger(subsystem: "ai.cleobot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "CleoBot is paused", message: "Unpause CleoBot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run CleoBot agent?", message: body) else { return }
        }
