...
enum CommandResolver {
    private static let projectRootDefaultsKey = "cleobot.gatewayProjectRootPath"
    private static let helperName = "cleobot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let cleobotEntry = root.appendingPathComponent("cleobot.mjs").path
        if FileManager().isReadableFile(atPath: cleobotEntry) { return cleobotEntry }
        let binEntry = root.appendingPathComponent("bin/cleobot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/cleobot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let cleobotPaths = self.cleobotManagedPaths(home: home)
        if !cleobotPaths.isEmpty {
            extras.insert(contentsOf: cleobotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + cleobotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func cleobotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".cleobot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func cleobotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCleoBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("cleobot.mjs").path,
            root.appendingPathComponent("bin/cleobot.js").path,
        ]
    ...
    static func hasAnyCleoBotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.cleobotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func cleobotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let cleobotPath = self.projectCleoBotExecutable(projectRoot: root) {
                return [cleobotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "cleobot", subcommand] + extraArgs
            }
            if let cleobotPath = self.cleobotExecutable(searchPaths: searchPaths) {
                return [cleobotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            cleobot entrypoint missing (looked for dist/index.js or cleobot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real cleobot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/cleobot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v cleobot >/dev/null 2>&1; then
          CLI="$(command -v cleobot)"
          cleobot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/cleobot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/cleobot.mjs"
            node "$PRJ/cleobot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/cleobot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/cleobot.js"
            node "$PRJ/bin/cleobot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent cleobot"
          pnpm --silent cleobot \(quotedArgs);
        else
          echo "cleobot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? CleoBotConfigFile.loadDict()
