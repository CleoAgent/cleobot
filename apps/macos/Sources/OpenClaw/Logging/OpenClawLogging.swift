...
enum CleoBotLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = CleoBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CleoBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CleoBotLogging.bootstrapIfNeeded()
        let label = CleoBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CleoBotOSLogHandler: LogHandler {
...
struct CleoBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CleoBotLogging.parseLabel(self.label)
