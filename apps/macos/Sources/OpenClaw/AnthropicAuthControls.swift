...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: CleoBotOAuthStore.AnthropicOAuthStatus = CleoBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: CleoBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = CleoBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = CleoBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try CleoBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. CleoBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: CleoBotOAuthStore.AnthropicOAuthStatus,
