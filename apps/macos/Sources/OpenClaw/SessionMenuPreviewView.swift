...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.cleobot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CleoBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CleoBotSessionPreviewEntry,
    ...
    private static func cache(payload: CleoBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CleoBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CleoBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CleoBotKit.AnyCodable]) -> [CleoBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CleoBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CleoBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CleoBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CleoBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CleoBotChatMessage) -> String? {
