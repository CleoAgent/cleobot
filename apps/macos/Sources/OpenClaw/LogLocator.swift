...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CLEOBOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/cleobot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("cleobot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("cleobot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/cleobot/ (rolling or stdout), or nil if none exist.
