...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CleoBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.cleobot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CleoBotKit.AnyCodable]()) {
                $0[$1.key] = CleoBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CleoBotProtocol.AnyCodable?) -> [String: CleoBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CleoBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CleoBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CleoBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("cleobot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("cleobot.control.agent")
}
