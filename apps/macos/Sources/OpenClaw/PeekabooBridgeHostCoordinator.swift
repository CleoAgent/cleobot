...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.cleobot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: CleoBotPeekabooBridgeServices?
    private static var cleobotSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("CleoBot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = CleoBotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.cleobotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.cleobotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class CleoBotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.cleobot.peekaboo")
