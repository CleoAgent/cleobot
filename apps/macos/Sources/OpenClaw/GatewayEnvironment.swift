...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.cleobot", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["CLEOBOT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = CleoBotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.cleobotExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "cleobot CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["CLEOBOT_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "cleobot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "cleobot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "cleobot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "cleobot@\(target)"])
            }

        statusHandler("Installing cleobot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed cleobot@\(target)")
        } else {
