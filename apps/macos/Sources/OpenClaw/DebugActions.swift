...
enum DebugActions {
    private static let verboseDefaultsKey = "cleobot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "cleobot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = CleoBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "CleoBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CleoBotPaths.configURL
        guard
