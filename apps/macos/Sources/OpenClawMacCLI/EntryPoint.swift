...
struct CleoBotMacCLI {
    static func main() async {
        ...
        default:
            fputs("cleobot-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      cleobot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      cleobot-mac discover [--timeout <ms>] [--json] [--include-local]
      cleobot-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      cleobot-mac connect
      cleobot-mac connect --url ws://127.0.0.1:18789 --json
      cleobot-mac discover --timeout 3000 --json
      cleobot-mac wizard --mode local
    """)
}
