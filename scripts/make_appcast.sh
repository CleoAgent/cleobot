...
ZIP=${1:?"Usage: $0 CleoBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/cleobot/openclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^CleoBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/cleobot/openclaw/releases/download/v${VERSION}/"}
