#!/bin/bash
# Setup CleoBot Auth Management System
...
echo "=== CleoBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'cleobot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  CleoBot message: Send warning via CleoBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/cleobot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/cleobot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/cleobot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now cleobot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status cleobot-auth-monitor.timer"
echo ""
echo "Quick commands:"
