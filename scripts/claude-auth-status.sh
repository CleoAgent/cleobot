...
# Checks both Claude Code and CleoBot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CLEOBOT_AUTH="$HOME/.cleobot/agents/main/agent/auth-profiles.json"
...
check_openclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$CLEOBOT_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CLEOBOT_AUTH" 2>/dev/null || echo "0")
    fi
...
echo ""
echo "CleoBot Auth (~/.cleobot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$CLEOBOT_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$CLEOBOT_AUTH" 2>/dev/null || echo "0")
...
if systemctl --user is-active openclaw >/dev/null 2>&1; then
    echo -e "CleoBot service: ${GREEN}running${NC}"
else
    echo -e "CleoBot service: ${RED}NOT running${NC}"
fi
