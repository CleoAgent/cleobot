...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/cleobot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "CleoBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/cleobot/scripts/mobile-reauth.sh'" --id cleobot-auth
        ;;
    *)
