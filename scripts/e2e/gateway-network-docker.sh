...
IMAGE_NAME="cleobot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="cleobot-net-e2e-$$"
GW_NAME="cleobot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CLEOBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "CLEOBOT_SKIP_CHANNELS=1" \
	  -e "CLEOBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "CLEOBOT_SKIP_CRON=1" \
	  -e "CLEOBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
