...
IMAGE_NAME="cleobot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/cleobot-bin:$PATH"
  mkdir -p /tmp/cleobot-bin
  cat > /tmp/cleobot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/cleobot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/cleobot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/cleobot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.cleobot/workspace"
    config_path="$HOME/.cleobot/openclaw.json"
    sessions_dir="$HOME/.cleobot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.cleobot/openclaw.json" <<'"'"'JSON'"'"'
{
