...
INSTALL_URL="${CLEOBOT_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CLEOBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CLEOBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openclaw"
PACKAGE_NAME="${CLEOBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CLEOBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CLEOBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CLEOBOT_INSTALL_LATEST_OUT:-}"
fi
