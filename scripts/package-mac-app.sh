...
# Build and bundle CleoBot into a minimal .app we can open.
# Outputs to dist/CleoBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/CleoBot.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/cleobot/openclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/CleoBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :CleoBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :CleoBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/CleoBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/CleoBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/CleoBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/CleoBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/CleoBot/Resources/CleoBot.icns" "$APP_ROOT/Contents/Resources/CleoBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/CleoBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying CleoBotKit resources"
CLEOBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/CleoBotKit_CleoBotKit.bundle"
if [ -d "$CLEOBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/CleoBotKit_CleoBotKit.bundle"
  cp -R "$CLEOBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/CleoBotKit_CleoBotKit.bundle"
else
  echo "WARN: CleoBotKit resource bundle not found at $CLEOBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running CleoBot"
killall -q CleoBot 2>/dev/null || true
