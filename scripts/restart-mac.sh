#!/usr/bin/env bash
# Reset CleoBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLEOBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="CleoBot.app/Contents/MacOS/CleoBot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/CleoBot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/CleoBot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/CleoBot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.openclaw.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/cleobot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLEOBOT_RESTART_LOG:-/tmp/cleobot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLEOBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.cleobot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CLEOBOT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      ...
      log "  rm ~/.cleobot/disable-launchagent"
      log ""
...
log "==> Killing existing CleoBot instances"
kill_all_openclaw
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product CleoBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/CleoBot.app" ]]; then
    APP_BUNDLE="/Applications/CleoBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/CleoBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/CleoBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/CleoBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLEOBOT_APP_BUNDLE to your installed CleoBot.app"
}
