...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "CleoBot")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "CleoBot")
...
DMG_TEMP="$(mktemp -d /tmp/cleobot-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/CleoBot/Resources/CleoBot.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/cleobot-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/cleobot-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/cleobot-dmg-limits.txt
