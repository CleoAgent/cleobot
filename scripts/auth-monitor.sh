...
#
# Suggested cron: */30 * * * * /home/admin/cleobot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send CleoBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., cleobot-alerts)
...
STATE_FILE="$HOME/.cleobot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via CleoBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via CleoBot to $NOTIFY_PHONE..."
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! CleoBot is down. Run: ssh l36 '~/cleobot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
