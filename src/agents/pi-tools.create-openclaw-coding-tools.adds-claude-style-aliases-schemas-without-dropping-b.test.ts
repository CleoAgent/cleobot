...
describe("createCleoBotCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createCleoBotCodingTools({
      config,
    ...
    const anthropicTools = createCleoBotCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createCleoBotCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createCleoBotCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
