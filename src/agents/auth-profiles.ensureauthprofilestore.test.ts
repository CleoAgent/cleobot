...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtemp(path.join(os.tmpdir(), "cleobot-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtemp(path.join(os.tmpdir(), "cleobot-auth-merge-"));
    ...
    try {
      ...
      process.env.CLEOBOT_AGENT_DIR = mainDir;
