...
const MODELS_CONFIG: CleoBotConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureCleoBotModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureCleoBotModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureCleoBotModelsJson } = await import("./models-config.js");
      const { resolveCleoBotAgentDir } = await import("./agent-paths.js");

      await ensureCleoBotModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveCleoBotAgentDir(), "models.json");
