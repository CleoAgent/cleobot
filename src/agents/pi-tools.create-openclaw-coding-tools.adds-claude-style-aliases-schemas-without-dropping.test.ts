...
import { createCleoBotTools } from "./openclaw-tools.js";
import { __testing, createCleoBotCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createCleoBotCodingTools();

describe("createCleoBotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createCleoBotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCleoBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "cleobot-outside.txt");
