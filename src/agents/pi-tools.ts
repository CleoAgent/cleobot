...
function resolveExecConfig(cfg: CleoBotConfig | undefined) {
...
export function createCleoBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createCleoBotReadTool(freshReadTool)];
    }
