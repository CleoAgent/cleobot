...
describe("createCleoBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-read-"));
    try {
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "cleobot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "cleobot-workspace"),
    ...
    const tools = createCleoBotCodingTools({ sandbox });
