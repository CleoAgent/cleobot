...
describe("buildWorkspaceSkillStatus", () => {
  it("reports missing requirements and install options", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-"));
  ...
  it("marks bundled skills blocked by allowlist", async () => {
    ...
    const originalBundled = process.env.CLEOBOT_BUNDLED_SKILLS_DIR;
    ...
    try {
      process.env.CLEOBOT_BUNDLED_SKILLS_DIR = bundledDir;
      const report = buildWorkspaceSkillStatus(workspaceDir, {
        managedSkillsDir: path.join(workspaceDir, ".managed"),
      ...
      if (originalBundled === undefined) {
        delete process.env.CLEOBOT_BUNDLED_SKILLS_DIR;
      } else {
        process.env.CLEOBOT_BUNDLED_SKILLS_DIR = originalBundled;
      }
    }
  });
