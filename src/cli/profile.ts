...
export function applyCliProfileEnv(params: {
  ...
  env.CLEOBOT_PROFILE = profile;

  const stateDir = env.CLEOBOT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.CLEOBOT_STATE_DIR?.trim()) {
    env.CLEOBOT_STATE_DIR = stateDir;
  }

  if (!env.CLEOBOT_CONFIG_PATH?.trim()) {
    env.CLEOBOT_CONFIG_PATH = path.join(stateDir, "cleobot.json");
  }

  if (profile === "dev" && !env.CLEOBOT_GATEWAY_PORT?.trim()) {
    env.CLEOBOT_GATEWAY_PORT = "19001";
  }
}
