...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.CLEOBOT_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.CLEOBOT_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.CLEOBOT_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.CLEOBOT_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token (or CLEOBOT_GATEWAY_TOKEN), or pass --token.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or CLEOBOT_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (bind !== "loopback" && !hasSharedSecret) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or CLEOBOT_GATEWAY_TOKEN/CLEOBOT_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: CLEOBOT_GATEWAY_TOKEN env if set)",
    )
