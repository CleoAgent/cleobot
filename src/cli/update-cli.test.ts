...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(resolveCleoBotPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("CleoBot update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    ...
    try {
      ...
      vi.mocked(resolveCleoBotPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    try {
      setTty(true);
      process.env.CLEOBOT_GIT_DIR = tempDir;
      ...
      process.env.CLEOBOT_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
