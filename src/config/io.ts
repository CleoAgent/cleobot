...
import type { CleoBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareCleoBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): CleoBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as CleoBotConfig;
}
...
function stampConfigVersion(cfg: CleoBotConfig): CleoBotConfig {
...
function warnIfConfigFromFuture(cfg: CleoBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareCleoBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer CleoBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: CleoBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CleoBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as CleoBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as CleoBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: CleoBotConfig) {
    clearConfigCache();
...
// module scope. `CLEOBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CLEOBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CLEOBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): CleoBotConfig {
...
export async function writeConfigFile(cfg: CleoBotConfig): Promise<void> {
  clearConfigCache();
