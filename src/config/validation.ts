...
import type { CleoBotConfig, ConfigValidationIssue } from "./types.js";
...
import { CleoBotSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: CleoBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: CleoBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CleoBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as CleoBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as CleoBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as CleoBotConfig)),
    ),
  };
}
