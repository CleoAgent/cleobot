...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from CLEOBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CLEOBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CLEOBOT_STATE_DIR = "/custom/state";
    try {
