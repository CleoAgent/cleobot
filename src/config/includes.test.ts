...
function etcCleoBotPath(...parts: string[]) {
  return path.join(ETC_CLEOBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcCleoBotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "cleobot.json"))).toEqual({
      shared: true,
    });
  });
});
