...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.CLEOBOT_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.CLEOBOT_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.CLEOBOT_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.CLEOBOT_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.CLEOBOT_TEST_LAUNCHCTL_LOG = logPath;
    process.env.CLEOBOT_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        CLEOBOT_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.CLEOBOT_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.CLEOBOT_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.CLEOBOT_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.CLEOBOT_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when CLEOBOT_PROFILE is default", () => {
  ...
  it("uses default label when CLEOBOT_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when CLEOBOT_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", CLEOBOT_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.openclaw.jbphoenix.plist",
    );
  });

  it("prefers CLEOBOT_LAUNCHD_LABEL over CLEOBOT_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      CLEOBOT_PROFILE: "jbphoenix",
      CLEOBOT_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from CLEOBOT_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      CLEOBOT_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty CLEOBOT_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      CLEOBOT_PROFILE: "myprofile",
      CLEOBOT_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from CLEOBOT_PROFILE", () => {
    const env = { HOME: "/Users/test", CLEOBOT_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.openclaw.myprofile.plist",
    );
  });
