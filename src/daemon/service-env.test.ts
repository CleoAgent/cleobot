...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CLEOBOT_GATEWAY_PORT).toBe("18789");
    expect(env.CLEOBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.CLEOBOT_SERVICE_MARKER).toBe("openclaw");
    expect(env.CLEOBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CLEOBOT_SERVICE_VERSION).toBe("string");
    expect(env.CLEOBOT_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.CLEOBOT_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", CLEOBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.CLEOBOT_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.CLEOBOT_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
});
