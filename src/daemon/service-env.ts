...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CLEOBOT_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.CLEOBOT_STATE_DIR;
  const configPath = env.CLEOBOT_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    CLEOBOT_PROFILE: profile,
    CLEOBOT_STATE_DIR: stateDir,
    CLEOBOT_CONFIG_PATH: configPath,
    CLEOBOT_GATEWAY_PORT: String(port),
    CLEOBOT_GATEWAY_TOKEN: token,
    CLEOBOT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CLEOBOT_SYSTEMD_UNIT: systemdUnit,
    CLEOBOT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CLEOBOT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    CLEOBOT_STATE_DIR: stateDir,
    CLEOBOT_CONFIG_PATH: configPath,
    CLEOBOT_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CLEOBOT_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CLEOBOT_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CLEOBOT_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CLEOBOT_LOG_PREFIX: "node",
    CLEOBOT_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CLEOBOT_SERVICE_KIND: NODE_SERVICE_KIND,
    CLEOBOT_SERVICE_VERSION: VERSION,
  };
}
