...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cleobot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtemp(path.join(os.tmpdir(), "cleobot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtemp(path.join(os.tmpdir(), "cleobot-lid-a-"));
    const second = fs.mkdtemp(path.join(os.tmpdir(), "cleobot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cleobot-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/cleobot")).toBe(path.resolve(os.homedir(), "openclaw"));
  });
