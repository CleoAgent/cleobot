...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.CLEOBOT_SKIP_BROWSER_CONTROL_SERVER,
    token: process.env.CLEOBOT_GATEWAY_TOKEN,
    password: process.env.CLEOBOT_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.CLEOBOT_STATE_DIR = prev.stateDir;
    process.env.CLEOBOT_CONFIG_PATH = prev.configPath;
    process.env.CLEOBOT_SKIP_CHANNELS = prev.skipChannels;
    process.env.CLEOBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.CLEOBOT_SKIP_CRON = prev.skipCron;
    process.env.CLEOBOT_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.CLEOBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.CLEOBOT_GATEWAY_TOKEN = prev.token;
    process.env.CLEOBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CLEOBOT_STATE_DIR = stateDir;
    process.env.CLEOBOT_CONFIG_PATH = path.join(stateDir, "cleobot.json");
