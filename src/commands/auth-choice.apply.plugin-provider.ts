...
function applyDefaultModel(cfg: CleoBotConfig, model: string): CleoBotConfig {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveCleoBotAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
