...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ CLEOBOT_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ CLEOBOT_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.CLEOBOT_PORT).toBe("3000");
