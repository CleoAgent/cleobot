...
beforeEach(() => {
  ...
  resolveCleoBotPackageRoot.mockReset().mockResolvedValue(null);
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CLEOBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CLEOBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.CLEOBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveCleoBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadCleoBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  ...
  it("skips gateway auth warning when CLEOBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/cleobot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.CLEOBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
