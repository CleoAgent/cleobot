...
export function listAgentEntries(cfg: CleoBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: CleoBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: CleoBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CleoBotConfig): AgentSummary[] {
