...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCleoBotAgentDir: vi.fn().mockReturnValue("/tmp/openclaw-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveCleoBotAgentDir: mocks.resolveCleoBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCleoBotAgentDir).toHaveBeenCalled();
