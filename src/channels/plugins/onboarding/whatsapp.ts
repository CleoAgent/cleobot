...
function setWhatsAppDmPolicy(cfg: CleoBotConfig, dmPolicy: DmPolicy): CleoBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: CleoBotConfig, allowFrom?: string[]): CleoBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: CleoBotConfig, selfChatMode: boolean): CleoBotConfig {
...
async function detectWhatsAppLinked(cfg: CleoBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so CleoBot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for CleoBot" },
    ],
  });
