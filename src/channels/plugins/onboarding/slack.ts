...
function setSlackDmPolicy(cfg: CleoBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "CleoBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for CleoBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: CleoBotConfig, allowFrom: string[]): CleoBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
