...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "CLEOBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CLEOBOT_STATE_DIR = path.join(home, ".cleobot");
    ...
    await fs.mkdir(path.join(home, ".cleobot"), { recursive: true });
