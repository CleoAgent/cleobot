...
import { getSessionByToken, validateApiKey } from "../auth/index.js";
...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.CLEOBOT_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.CLEOBOT_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or CLEOBOT_GATEWAY_TOKEN)",
    );
  }
...
export async function authorizeGatewayConnect(params: {
  ...
  // Check cleobot_session cookie first (highest priority)
  if (req?.headers.cookie) {
    const cookieMatch = req.headers.cookie.match(/cleobot_session=([^;]+)/);
    if (cookieMatch) {
