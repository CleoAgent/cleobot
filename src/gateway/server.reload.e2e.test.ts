...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CLEOBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.CLEOBOT_SKIP_GMAIL_WATCHER;
    process.env.CLEOBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CLEOBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CLEOBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CLEOBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
