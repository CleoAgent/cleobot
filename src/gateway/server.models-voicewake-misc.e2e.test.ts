...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.CLEOBOT_STATE_DIR = path.join(homeDir, ".cleobot");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-home-"));
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".cleobot", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.CLEOBOT_CANVAS_HOST_PORT;
    process.env.CLEOBOT_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.CLEOBOT_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.CLEOBOT_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.CLEOBOT_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing CLEOBOT_CONFIG_PATH");
    }
