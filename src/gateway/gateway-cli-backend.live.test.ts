...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.CLEOBOT_LIVE_TEST);
const CLI_LIVE = isTruthyEnvValue(process.env.CLEOBOT_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.CLEOBOT_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.CLEOBOT_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("CLEOBOT_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.CLEOBOT_CONFIG_PATH,
      token: process.env.CLEOBOT_GATEWAY_TOKEN,
      skipChannels: process.env.CLEOBOT_SKIP_CHANNELS,
      skipGmail: process.env.CLEOBOT_SKIP_GMAIL_WATCHER,
      skipCron: process.env.CLEOBOT_SKIP_CRON,
      skipCanvas: process.env.CLEOBOT_SKIP_CANVAS_HOST,
    ...
    process.env.CLEOBOT_SKIP_CANVAS_HOST = "1";
    delete process.env.ANTHROPIC_API_KEY;
    delete process.env.ANTHROPIC_API_KEY_OLD;

    const token = `test-${randomUUID()}`;
    process.env.CLEOBOT_GATEWAY_TOKEN = token;

    const rawModel = process.env.CLEOBOT_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `CLEOBOT_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.CLEOBOT_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `CLEOBOT_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "CLEOBOT_LIVE_CLI_BACKEND_ARGS",
        process.env.CLEOBOT_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`CLEOBOT_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "CLEOBOT_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.CLEOBOT_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.CLEOBOT_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.CLEOBOT_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "CLEOBOT_LIVE_CLI_BACKEND_IMAGE_MODE requires CLEOBOT_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-live-cli-"));
    const disableMcpConfig = process.env.CLEOBOT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "cleobot.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.CLEOBOT_CONFIG_PATH = tempConfigPath;
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        ...
        process.env.CLEOBOT_CONFIG_PATH = previous.configPath;
      }
      if (previous.token === undefined) {
        delete process.env.CLEOBOT_GATEWAY_TOKEN;
      } else {
        process.env.CLEOBOT_GATEWAY_TOKEN = previous.token;
      }
      if (previous.skipChannels === undefined) {
        delete process.env.CLEOBOT_SKIP_CHANNELS;
      } else {
        process.env.CLEOBOT_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.CLEOBOT_SKIP_GMAIL_WATCHER;
      } else {
        process.env.CLEOBOT_SKIP_GMAIL_WATCHER = previous.skipGmail;
      }
      if (previous.skipCron === undefined) {
        delete process.env.CLEOBOT_SKIP_CRON;
      } else {
        process.env.CLEOBOT_SKIP_CRON = previous.skipCron;
      }
      if (previous.skipCanvas === undefined) {
        delete process.env.CLEOBOT_SKIP_CANVAS_HOST;
      } else {
        process.env.CLEOBOT_SKIP_CANVAS_HOST = previous.skipCanvas;
      }
