...
import { createCleoBotTools } from "../agents/openclaw-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.CLEOBOT_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.CLEOBOT_GATEWAY_PORT = String(gatewayPort);
  process.env.CLEOBOT_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CLEOBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createCleoBotTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
