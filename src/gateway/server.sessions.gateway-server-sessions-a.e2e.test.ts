...
beforeAll(async () => {
  previousToken = process.env.CLEOBOT_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.CLEOBOT_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-sessions-"));
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
