...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.CLEOBOT_STATE_DIR;
  previousConfigPath = process.env.CLEOBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.CLEOBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.CLEOBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.CLEOBOT_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CLEOBOT_STATE_DIR = path.join(tempHome, ".cleobot");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.CLEOBOT_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.CLEOBOT_CONFIG_PATH;
    } else {
      process.env.CLEOBOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.CLEOBOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.CLEOBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.CLEOBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CLEOBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.CLEOBOT_SKIP_CANVAS_HOST;
    } else {
      process.env.CLEOBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.CLEOBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CLEOBOT_GATEWAY_TOKEN = fallbackToken;
  }
