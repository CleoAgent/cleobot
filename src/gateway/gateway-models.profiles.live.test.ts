...
import type { CleoBotConfig, ModelProviderConfig } from "../config/types.js";
import { resolveCleoBotAgentDir } from "../agents/agent-paths.js";
...
import { ensureCleoBotModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.CLEOBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CLEOBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.CLEOBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<CleoBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.CLEOBOT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CLEOBOT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveCleoBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-live-state-"));
  ...
  process.env.CLEOBOT_AGENT_DIR = tempAgentDir;
  ...
  const agentDir = resolveCleoBotAgentDir();
  const sanitizedCfg: CleoBotConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "CleoBot live tool probe (local, safe): " +
    ...
    process.env.CLEOBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CLEOBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureCleoBotModelsJson(cfg);
      ...
      const rawModels = process.env.CLEOBOT_LIVE_GATEWAY_MODELS?.trim();
