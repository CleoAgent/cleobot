...
import { loadCleoBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.CLEOBOT_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CLEOBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadCleoBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CLEOBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadCleoBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadCleoBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
