...
import { discoverCleoBotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CleoBotPluginDefinition;
  register?: CleoBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as CleoBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CleoBotPluginDefinition;
...
export function loadCleoBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverCleoBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: CleoBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as CleoBotPluginModule;
    } catch (err) {
