...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-web-home-"));
...
const _makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-session-"));
...
describe("web auto-reply", () => {
  ...
  it("detects LID mentions using authDir mapping", async () => {
    ...
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-wa-auth-"));
