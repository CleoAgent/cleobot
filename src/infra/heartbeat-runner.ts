...
function resolveActiveHoursTimezone(cfg: CleoBotConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CleoBotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CleoBotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: CleoBotConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: CleoBotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CleoBotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CleoBotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CleoBotConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
