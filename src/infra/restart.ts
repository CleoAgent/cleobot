...
export function triggerCleoBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CLEOBOT_SYSTEMD_UNIT,
        process.env.CLEOBOT_PROFILE,
      );
  ...
  const label =
    process.env.CLEOBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CLEOBOT_PROFILE);
