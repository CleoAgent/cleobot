...
This directory contains hooks that ship with CleoBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.cleobot/workspace`)
...
**Output**: `~/.cleobot/logs/commands.log`
...
- **Managed hooks**: `~/.cleobot/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.cleobot/openclaw.json`:
