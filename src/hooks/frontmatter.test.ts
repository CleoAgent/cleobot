...
describe("resolveCleoBotMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    ...
    const result = resolveCleoBotMetadata(frontmatter);
    ...
    const frontmatter = {
      metadata: JSON.stringify({
        openclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with CleoBot" },
    ...
    "openclaw":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with CleoBot" }],
      },
  }
---
...
`;
    ...
    const openclaw = resolveCleoBotMetadata(frontmatter);
