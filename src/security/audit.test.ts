...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "cleobot.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "cleobot.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: CleoBotConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.CLEOBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-discord-"));
    process.env.CLEOBOT_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.CLEOBOT_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "cleobot-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.CLEOBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.CLEOBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-discord-open-"));
    process.env.CLEOBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: CleoBotConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.CLEOBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-slack-"));
    process.env.CLEOBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: CleoBotConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.CLEOBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-slack-open-"));
    process.env.CLEOBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: CleoBotConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.CLEOBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-telegram-"));
    process.env.CLEOBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: CleoBotConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: CleoBotConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.CLEOBOT_GATEWAY_TOKEN;
    process.env.CLEOBOT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: CleoBotConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.CLEOBOT_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: "/Users/test/Dropbox/.cleobot/cleobot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cleobot-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: CleoBotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "cleobot.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.CLEOBOT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.CLEOBOT_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.CLEOBOT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.CLEOBOT_GATEWAY_PASSWORD;
      } else {
        process.env.CLEOBOT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.CLEOBOT_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: CleoBotConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.CLEOBOT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: CleoBotConfig = {
        gateway: {
          mode: "remote",
