...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    try {
      ...
      expect(def.name).toBe(DEFAULT_CLEOBOT_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_CLEOBOT_BROWSER_PROFILE_NAME);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_CLEOBOT_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
...
describe("browser chrome helpers", () => {
  ...
  it("stopCleoBotChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopCleoBotChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopCleoBotChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopCleoBotChrome sends SIGTERM and returns once CDP is down", async () => {
