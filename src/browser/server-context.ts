...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the CleoBot Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchCleoBotChrome(current.resolved, profile);
    ...
    await stopCleoBotChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchCleoBotChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the CleoBot Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
  ...
  const resetProfile = async () => {
    ...
    const userDataDir = resolveCleoBotUserDataDir(profile.name);
    const profileState = getProfileState();
