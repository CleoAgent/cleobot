...
import type { CleoBotConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: CleoBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: CleoBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as CleoBotConfig),
...
export function getCommandDetection(_cfg?: CleoBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: CleoBotConfig) {
