...
import { scheduleGatewaySigusr1Restart, triggerCleoBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting CleoBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerCleoBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting CleoBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
