---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for CleoBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, CleoBot cron scheduling for periodic checks, or version status checks on a machine running CleoBot (laptop, workstation, Pi, VPS).
---

# CleoBot Host Hardening

## Overview

Assess and harden the host running CleoBot, then align it to a user-defined risk tolerance without breaking access. Use CleoBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CleoBot changes the host firewall, SSH, or OS updates; it does not.
...
5. CleoBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CleoBot can access sensitive data.
...
### 2) Run CleoBot security audits (read-only)

As part of the default read-only checks, run `cleobot security audit --deep`. Only offer alternatives if the user requests them:

1. `cleobot security audit` (faster, non-probing)
2. `cleobot security audit --json` (structured output)

Offer to apply CleoBot safe defaults (numbered):

1. `cleobot security audit --fix`

Be explicit that `--fix` only tightens CleoBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CleoBot version/update status (read-only)

As part of the default read-only checks, run `cleobot update status`.
...
- Credential hygiene notes (location of CleoBot creds, prefer disk encryption)
...
After CleoBot install or first hardening pass, run at least one baseline audit and version check:

- `cleobot security audit`
- `cleobot security audit --deep`
- `cleobot update status`

Ongoing monitoring is recommended. Use the CleoBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `cleobot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `cleobot update status`

Also offer a periodic version check so the user can decide when to update (numbered):

1. `cleobot update status` (preferred for source checkouts and channels)
2. `npm view cleobot version` (published npm version)

## CleoBot command accuracy

Use only supported commands and flags:

- `cleobot security audit [--deep] [--fix] [--json]`
- `cleobot status` / `cleobot status --deep`
- `cleobot health --json`
- `cleobot update status`
- `cleobot cron add|list|runs|run`

Do not invent CLI flags or imply CleoBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by CleoBot compaction:
