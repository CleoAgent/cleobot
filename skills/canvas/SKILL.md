...
Display HTML content on connected CleoBot nodes (Mac app, iOS, Android).
...
```bash
cat ~/.cleobot/openclaw.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.cleobot/openclaw.json | jq '.gateway.bind'`
...
Node is offline. Use `cleobot nodes list` to find online nodes.
